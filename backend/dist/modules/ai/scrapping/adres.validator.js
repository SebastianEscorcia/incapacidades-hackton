...
function buildResponse({ status, message, reason = null, document, expectedEps = null, scraping = true, data = null, }) {
    return {
        status,
        payload: {
            estado: status,
...
function validateAdresResult(result, expectedEps) {
    ...
    const epsMatch = affiliationRows.some((row) => Object.entries(row || {}).some(([key, value]) => {
        const normalizedKey = normalizeText(key);
        const normalizedValue = normalizeText(value);
        const isEpsField = normalizedKey.includes('eps') ||
...
async function getFormFrame(page) {
    for (const frame of page.frames()) {
        if (frame.url().includes(FORM_FRAME_URL))
            return frame;
    }
    return null;
}
async function extractData(frameOrPage) {
    return frameOrPage.evaluate(() => {
        ...
        if (fm)
            data.fechaImpresion = fm[1].trim();
        const em = text.match(/Estaci[oó]n de origen[:\s]+([0-9.]+)/i);
        if (em)
            data.estacionOrigen = em[1].trim();
        return data;
    });
}
async function adresValidator(docNumber, expectedEps = null) {
    ...
    try {
        ...
        if (!formFrame)
            throw new Error('Iframe del formulario no encontrado.');
        ...
        const hasChallenge = page
            .frames()
            .some((f) => f.url().includes('recaptcha') && f.url().includes('bframe'));
        if (hasChallenge) {
...
exports.default = adresValidator;
//# sourceMappingURL=adres.validator.js.map