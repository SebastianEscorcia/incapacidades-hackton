...
function validateRethusResult(textResult = '') {
  ...
  if (negativePatterns.some((pattern) => pattern.test(normalized))) {
    return {
      ...
      reason:
        'La respuesta oficial de RETHUS no reporta registro vigente para el documento consultado.',
    };
  }
  ...
  return {
    ...
    reason:
      'La respuesta de RETHUS fue ambigua y no pudo clasificarse de forma confiable.',
  };
}
...
async function rethusValidator(docNumber: string) {
  ...
  try {
    ...
    if (!captchaInputId) {
      throw new Error(
        'No se pudo hacer el web scraping: no se encontro el campo para ingresar CAPTCHA.',
      );
    }
