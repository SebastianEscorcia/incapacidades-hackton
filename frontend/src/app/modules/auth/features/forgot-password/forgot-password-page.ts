    ...
    <main class="auth-page">
      <section class="auth-card">
        ...
        <div class="card-heading">
          <span class="icon"><i class="pi pi-envelope"></i></span>
          <div>
            <h1>{{ 'pages.forgotPassword' | translate }}</h1>
            <p>{{ 'pages.forgotPasswordDescription' | translate }}</p>
          </div>
        </div>
        ...
        <form [formGroup]="form" (ngSubmit)="submit()" class="form">
          ...
          <p-button type="submit" [disabled]="form.invalid" styleClass="submit-button">
            ...
            <span>{{ 'auth.forgotAction' | translate }}</span>
          </p-button>
        </form>

        <div class="secondary-actions">
          <a routerLink="/auth/login">Volver a iniciar sesión</a>
          <a routerLink="/auth/recover-password">Ya tengo un código</a>
